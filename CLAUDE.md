...
- **Multi-tab** — each open curl is an independent **session** (`makeSession()`); `state` always points at the active one and is reassigned on tab switch (every helper reads `state`). The Request Tabs Bar (`#crTabBar`) shows one tab per session (method badge + path + close ×) plus a **+** to open a new request. Open tabs persist to `chrome.storage.local` (`curl-tabs`/`curl-active`) **without responses** (responses are kept in memory only). `persistTabsSoon()` debounces edit-driven saves.
